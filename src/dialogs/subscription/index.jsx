...
export default function SetUpSubscription({ user, ...props }) {
    const prem = user.stats.premium || {}, tools = useShow();
    ...
    return (
        <Modal
            onSubmit={async (e) => {
                try {
                    ...
                    props.setAuth({ prem: res.data.leaderboard || { ...prem, ...e.values } });
                    e.hidePopUp();
                    // refreshStats();
                } catch (err) {
        ...
        >
            <div>
                ...
                <Input defaultValue={prem.price || 5} required={true} min={5} errorText="Minimum Price should be $5!" name='price' placeholder='Enter Amount' type='number' 
                    // startAdornment="$"
                 />
            </div>
            <div>
